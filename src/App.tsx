...
// Provided by Vite define in dev only
declare const __DEV_GEMINI_API_KEY__: string | undefined;
...
const DemoSection: React.FC<{ apiKey: string | null; onRequireKey: () => void }> = ({ apiKey, onRequireKey }) => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (isLoading || (!input.trim() && !pdfFile) || !apiKey) return;
    ...
    try {
      ...
      if (currentPdfFile) {
        ...
        result = await run(apiKey, currentInput, filePart);
      } else {
        result = await run(apiKey, currentInput);
      }
    ...
    setIsLoading(false);
  }, [input, isLoading, pdfFile, apiKey, disabled, onRequireKey]);
  ...
  return (
    <section className="py-12 sm:py-16">
      ...
      <div className="relative max-w-2xl mx-auto bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 shadow-2xl flex flex-col h-[600px]">
        {!apiKey && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-900/80 rounded-2xl">
            <div className="text-center space-y-4">
              <p className="text-slate-300">Enter your Gemini API key to enable the demo.</p>
              <button onClick={onRequireKey} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-md">Enter API Key</button>
            </div>
          </div>
        )}
        ...
        <div className="p-4 border-t border-slate-700/50">
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => (apiKey ? fileInputRef.current?.click() : onRequireKey())} className="text-slate-400 hover:text-cyan-400 p-3 rounded-full transition-colors duration-300 flex-shrink-0" disabled={isLoading}>
            ...
            <input
              ...
              onFocus={() => { if (!apiKey) onRequireKey(); }}
...
const Header: React.FC<{ onManageKey: () => void }> = ({ onManageKey }) => (
  <header className="py-12 text-center bg-gradient-to-b from-slate-900 to-transparent">
    <div className="flex justify-end container mx-auto px-4">
      <button onClick={onManageKey} className="text-sm px-3 py-1.5 rounded-md bg-slate-800 border border-slate-700 text-slate-200 hover:bg-slate-700">Manage API Key</button>
    </div>
...
const ApiKeyModal: React.FC<{ initialKey: string; onClose: () => void; onSave: (key: string) => void; onClear: () => void }> = ({ initialKey, onClose, onSave, onClear }) => {
  const [key, setKey] = useState(initialKey);
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-xl p-6 space-y-4">
        <h3 className="text-xl font-semibold text-white">Enter Gemini API Key</h3>
        <input
          type="password"
          value={key}
          onChange={(e) => setKey(e.target.value)}
          placeholder="AIza..."
          className="w-full bg-slate-800 text-white rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-600"
        />
        <div className="flex gap-2 justify-end">
          <button className="px-4 py-2 rounded-md border border-slate-700 text-slate-300 hover:bg-slate-800" onClick={onClose}>Cancel</button>
          <button className="px-4 py-2 rounded-md bg-cyan-600 hover:bg-cyan-500 text-white" onClick={() => onSave(key)}>Save</button>
          <button className="px-4 py-2 rounded-md bg-red-600 hover:bg-red-500 text-white" onClick={onClear}>Clear</button>
        </div>
        <p className="text-xs text-slate-400">Your key is stored in sessionStorage for this session only.</p>
      </div>
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  // Auto-seed API key in dev if present in .env (not used in production)
  useEffect(() => {
    const current = sessionStorage.getItem('GEMINI_API_KEY');
    if (!current && typeof __DEV_GEMINI_API_KEY__ !== 'undefined' && __DEV_GEMINI_API_KEY__) {
      sessionStorage.setItem('GEMINI_API_KEY', __DEV_GEMINI_API_KEY__);
      setApiKey(__DEV_GEMINI_API_KEY__);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen text-slate-100 bg-gradient-to-br from-gray-900 via-slate-900 to-indigo-950 selection:bg-cyan-500/30">
      <main className="container mx-auto px-4">
        <Header onManageKey={openKeyModal} />
        <MTRInfographicSection />
        <DemoSection apiKey={apiKey} onRequireKey={openKeyModal} />
        <ReferenceSection />
      </main>
      {showKeyModal && (
        <ApiKeyModal
